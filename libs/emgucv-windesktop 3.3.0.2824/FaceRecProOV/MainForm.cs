...
namespace MultiFaceRec
{
    public partial class FrmPrincipal : Form
    {
        ...
        private void button2_MouseCaptureChanged_1(object sender, EventArgs e)
        {
        ...
        private void button2_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                // per cambiare la risoluzione del frame modificare i primi due parametri del metodo Resize()
                ...
                // cambio la risoluzione dell'immagine e aggiungo il nuovo nome alla lista.
                TrainedFace = result.Resize(100, 100, Emgu.CV.CvEnum.INTER.CV_INTER_CUBIC);
                trainingImages.Add(TrainedFace);
                labels.Add(textBox1.Text);
                n = textBox1.Text;

                // Mostro l'immagine nel form.
                imageBox1.Image = TrainedFace;

                // Riscrivo il numero di facce trovate
                File.WriteAllText(Application.StartupPath + "/TrainedFaces/TrainedLabels.txt", trainingImages.ToArray().Length.ToString() + "%");


                for (int i = 1; i < trainingImages.ToArray().Length + 1; i++)
                {
                    // salvo un' immagine bitmap per ogni faccia trovata
                    trainingImages.ToArray()[i - 1].Save(Application.StartupPath + "/TrainedFaces/face" + i + ".bmp");
                    // scrivo sul file i nomi di tutte le facce trovate.
